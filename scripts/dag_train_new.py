...
def finalize_and_log():
    ...
    # Model selection with safe parameter unpacking
    if best_model_type == "RandomForest":
        ...
        final_model = ExtraTreesRegressor(
            n_estimators=best_params.get("et_n_estimators", 100),
            max_depth=best_params.get("et_max_depth", None),
            min_samples_split=best_params.get("et_min_samples_split", 2),
            random_state=SEED, n_jobs=-1
        )
        ...
        raise ValueError(f"Unsupported final model type: {best_model_type}")
